...
const (
	...
	// PunctuationPattern 匹配标点符号
	PunctuationPattern = `\p{P}`
	// SentenceEndPunctuationPattern 正则表达式用于匹配句子末尾的标点符号
	SentenceEndPunctuationPattern = `[.!?。！？¡¿,，;；]$`
	// SymbolsAndNumbersPattern 特殊符号和阿拉伯数字
	SymbolsAndNumbersPattern = `[\p{P}\p{N}]+`
)
...
var (
	...
	SentenceEndPunctuationRegex = regexp.MustCompile(SentenceEndPunctuationPattern)
	SymbolsAndNumbersRegex      = regexp.MustCompile(SymbolsAndNumbersPattern)
)
